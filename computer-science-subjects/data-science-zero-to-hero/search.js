...
(function () {
  ...
  async function loadIndex() {
    ...
    // Preferred path: data embedded inline by build.py via search-data.js.
    // Works even when the site is opened via file:// (fetch is blocked by CORS).
  ...
  function render(results, query) {
    if (!results.length) {
      hits.innerHTML = '<div class="empty">nessun risultato per "' + escapeHtml(query) + '"</div>';
